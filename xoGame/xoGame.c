...
/*
 * Description:
 *  This program implements the classic Tic-Tac-Toe game in C.
 ...
 * The game ends when a player wins or if the grid is full, resulting in a draw.
 *
 * Main features:
 * - Two-player gameplay where players alternate turns.
 * - Display of the 3x3 grid after each move.
 * - Real-time input validation to prevent moves on already occupied spaces.
 * - Check for a winner after every move.
 * - Clear indication of a win (three consecutive marks in a row, column, or diagonal).
 * - Draw detection when all spaces are filled without a winner.
 * - The game prompts for replay after each game ends.
 *
 * Instructions:
 * - Players take turns entering the position (1-9) to place their marks.
 * - The positions on the grid correspond to numbers 1-9, from top-left to bottom-right.
 * - The game ends when a player wins or if the grid is filled, resulting in a draw.
...
// Wrapper function to call `playTicTacToe` with the default COMPUTER turn
void playXo()
{
